...
int main(int narg, char const *argv[])
{
	...
	/*No aseguramos que no se una entrada de  nombre largo y que sea un archivo*/
		if((entry.attributes!= 0x0F) && (entry.attributes & 0x20)){
			...
			while(1){
				//como se encuentra en desorden siempre tengo que tener el offset para situarme y luego depende de lo otro
				offset=512+cluster*2; //512 para irme a la tabla FAT +cluster que estoy buscando
